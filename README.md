...
The onboarding error card and Settings screen can copy or clear a persistent diagnostic log. It records app/device versions, setup phases, the shell receiver and accessibility processing stages, gesture-policy decisions, action type and success state. Pairing codes, ADB private keys, HTTP endpoints, app package names, and configured URLs are never written to the log.
