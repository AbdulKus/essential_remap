...
Input and delivery have separate threads, bounded queues, stale-event rejection and duplicate-action receipts. Reader failures trigger a limited number of recovery attempts. If Android kills the helper itself, restart the monitor explicitly. During normal idle operation there is no polling, network traffic, heartbeat, wakeup alarm or CPU wake lock. Short partial wake locks cover the input handoff, gesture and action only; they do not light the display. Allow unrestricted battery use in Android settings for deep-sleep handling. The status checks a live connection rather than just remembering a successful installation.

**Keep USB debugging enabled when using the sleep monitor without Wi-Fi; no USB cable is needed.** Android can stop adbd and its descendant processes when both debugging transports are disabled. Restart the monitor after changing debugging settings if it stopped. See [Wi-Fi loss diagnosis](docs/screen-off-testing.md#wi-fi-loss-and-the-adb-daemon-lifetime).

When a release changes the monitor revision, restart the sleep monitor once in Settings. Updating the APK alone does not replace an already running shell process. Device verification steps are in [screen-off testing](docs/screen-off-testing.md).
...
Чтение и доставка разделены; очереди ограничены, устаревшие события отбрасываются, повторное выполнение защищено подтверждениями. При сбое чтения выполняется ограниченное число попыток восстановления. Если Android завершил сам helper-процесс, монитор нужно перезапустить. В обычном простое нет опроса, сетевого трафика, heartbeat, будильников или CPU wake lock. Короткая блокировка сна покрывает только передачу нажатия, распознавание и действие; экран от неё не загорается. Для работы в глубоком сне разрешите приложению использование батареи без ограничений. Статус проверяет живое соединение, а не только факт прошлой установки.

**Для работы монитора без Wi-Fi оставьте «Отладку по USB» включённой; кабель не нужен.** Если отключены обе отладки, Android может завершить adbd вместе с его дочерними процессами. Если монитор остановился после изменения настроек отладки, перезапустите его. Подробности: [диагностика отключения Wi-Fi](docs/screen-off-testing.md#wi-fi-loss-and-the-adb-daemon-lifetime).

После обновления, меняющего ревизию монитора, один раз перезапустите его в настройках. Обновление APK само по себе не заменяет уже работающий shell-процесс. Сценарии проверки описаны в [screen-off testing](docs/screen-off-testing.md).
